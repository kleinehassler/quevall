...
CREATE  VIEW varti AS 
 SELECT a.icode AS codigo, a.iname AS nombre, a.artpeso AS peso, a.artcosto AS costo, a.saldocon, a.isiva AS carga_iva, 
 	...
 	p.descripda as promocion, b.descripda as ubipro, e.descripda as ubica2, t.descripda as cntbulto, a.pvp
   FROM items a
   LEFT JOIN detagru u ON a.iunidad = u.iddato
   LEFT JOIN monedas m ON a.imoney = m.mocode
   LEFT JOIN detagru c ON a.artmarca = c.iddato
...
CREATE VIEW varticulos AS
    ...
    a.costopro, a.artsgrupo, b.descripda as ubipro, e.descripda as ubica2, t.descripda as cntbulto, a.pvp  
...
CREATE  VIEW vitems AS 
 SELECT a.iditem, a.artgrupo, a.artsgrupo, a.artipo, a.artmarca, a.artpeso, a.artsell, a.artcosto, a.artfob, a.artcosto2, 
 	...
 	t.descripda as cntbulto,  a.isserie,
...
CREATE VIEW vauxcon AS
    SELECT DISTINCT a.tipo, a.idauxiliar, (substr(a.auxiliar, 2, 8))::character(7) AS auxiliar, a.auxname 
    FROM (((SELECT DISTINCT 'B'::bpchar AS tipo, c.idcuenta AS idauxiliar, to_char(c.numaux, ('0999999'::bpchar)::text) AS auxiliar, 
    			((btrim((b.descripda)::text, ' '::text) || ' '::text) || ((c.nucuban)::character varying(15))::text) AS auxname 
    		FROM detagru b, ctasban c 
    		WHERE (c.idbanco = b.iddato) 
    		ORDER BY 1, c.idcuenta, to_char(c.numaux, ('0999999'::bpchar)::text), ((btrim((b.descripda)::text, ' '::text) || ' '::text) || ((c.nucuban)::character varying(15))::text)) 
    		UNION 
    		(SELECT DISTINCT 'S'::bpchar AS tipo, s.idsujeto AS idauxiliar, to_char(s.scode, ('0999999'::bpchar)::text) AS auxiliar, 
    			CASE WHEN (char_length(btrim((s.sname)::text, ' '::text)) = 0) THEN s.ssri ELSE s.sname END AS auxname 
    		FROM sujetos s 
    		ORDER BY 1, s.idsujeto, to_char(s.scode, ('0999999'::bpchar)::text), CASE WHEN (char_length(btrim((s.sname)::text, ' '::text)) = 0) THEN s.ssri ELSE s.sname END)) 
    		UNION 
    		(SELECT DISTINCT 'I'::bpchar AS tipo, i.iditem AS idauxiliar, to_char(i.icode, ('0999999'::bpchar)::text) AS auxiliar, 
    			i.iname AS auxname 
    		FROM vitems i 
    		ORDER BY 1, i.iditem, to_char(i.icode, ('0999999'::bpchar)::text), i.iname)) a 
    ORDER BY a.tipo, (substr(a.auxiliar, 2, 8))::character(7), a.idauxiliar, a.auxname;
...
CREATE VIEW vplancta AS
	...
	p.idplancuenta, p.idcuenta, p.pdocode, auxsuj, auxite, 
...
CREATE  VIEW vdocui AS 
 SELECT t.code, i.iditem, i.icode, i.iname, i.marca, i.grupo, i.subgrupo, i.itag, i.genero, i.itipo, i.iname AS auxiliar, 
 	...
 	i.pvp, i.factorc, i.numeconver, i.denoconver, i.isiva as isivai, i.ubipro, i.ubica2, i.isserie
   FROM detadoci t
...
CREATE  VIEW vdocusmall AS 
 SELECT DISTINCT d.code, d.userstore, d.usercaja, d.num, d.cliente, d.fecha, d.fto, d.pdocode, d.idsecudoc, d.seller, 
 		...
 		t.sname as nombret, v.isdevo, v.istransfer, d.casher, e.scode as codprin, 0 as codgar, e.sname as principal, '' as garante,
   ...
   LEFT JOIN sujetos t ON d.sujter  = t.idsujeto
   LEFT JOIN sujetos e on u.codprin=e.idsujeto;
...
d.numauto2 AS numautadq, d.fecha2 AS fecdocadq, d.fcaducidad2 AS feccadadq, s.descrip AS tipdoc, 
u.fecha AS feccom, u.sname, u.ssri, u.sruc, u.scedula, u.saddr, u.saddr2, u.sciudad, u.stelf, 
r.rubname, r.rubtab, r.valcal, c.valor, c.basecal AS monto, v.nomdoc, i.codigo AS codigoret, 
...
CREATE VIEW vsuj AS 
 SELECT s.idsujeto, s.scode, s.sname, s.ssri, s.sruc, s.scedula, s.spasaporte, s.isempleado, s.isproductor, s.isproveedor, 
 		s.iscliente, s.issocio, c.descripda AS ciudad, p.descripda AS provincia, s.diasplz, s.svendedor, s.srepre, s.scel, 
 		s.tipcli, s.tipact, t.descripda AS tipo_cli, a.descripda AS tipo_act, s.stag, s.sciudad, s.sprov, s.saddr, s.stelf, 
 		b.idsujeto as idprin, b.sname as principal, 0 as idgar, '' as garante, s.activo, s.scredito, s.scheques, s.sfax, 
 		s.calific, s.observ, s.sciudad2, u.descripda as ciudad2, r.descripda as parroquia, e.descripda as sector, s.saddr2,
...
CREATE VIEW vsujetos AS
    ...
    s.idsujeto, b.sname as principal, '' as garante, b.idsujeto as idprin, 0 as idgar, s.diasplz, s.srepre,
    s.scredito, s.scheques, s.calific, s.observ, u.descripda as ciudad2, t.descripda as parroquia, e.descripda as sector, 
...
CREATE OR REPLACE VIEW vadqing AS 
 SELECT a.iditem, a.fecha, a.punitario, a.costoprom
   FROM (         SELECT s.iditem, p.fecini AS fecha, s.cospro AS punitario, s.precio AS costoprom
              ...
              where not fecini isnull
        UNION all
                 SELECT t.iditem, d.fecha, max((t.punitario*t.qty - t.descuento) / t.qty) AS punitario, max(t.costoprom) AS costoprom
                   FROM detadoci t
              LEFT JOIN vdocusmall d ON t.code = d.code
             WHERE d.isaccount AND NOT d.isanulado AND (d.iddocu IN ( SELECT DISTINCT d.iddocu
                      FROM gdoc g, dgdoc t, docuse d
...
CREATE OR REPLACE VIEW vdocxml1 AS  SELECT DISTINCT d.idugra,    d.fecgra,    d.code,    d.succode,    d.depcode,    d.usercaja,    d.userstore,    d.num,    d.idsecudoc,    d.cliente,    d.fecha,    d.pdocode,    d.ffrom,    d.fto,    d.seller,    d.casher,    d.linkdoc,    d.sujter,    d.auxitem,    d.duracion,    d.taza,    d.valcuota,    d.numcuota,    d.numcajas,    d.poriva,    d.descuconiva,    d.descusiniva,    d.recargo,    d.recargo2,    d.recargo3,    d.flete,    d.subconiva,    z.tipitem,    d.chofer,    f.sname AS chname,    f.scedula AS chcedula,    f.sruc AS chruc,    d.nrec,    d.comenanu,    d.subsiniva,    d.subtotal,    d.ivavalor,    d.ivabienes,    d.icevalor,    d.ipsvalor,    d.montototal,    d.saldo,    d.tmpsaldo,    d.isguia,    d.isupdate,    d.islock,    d.isprint,    d.isaccount,    d.isbodega,    d.ispagado,    d.isanulado,    d.isautorizado,    d.isreversado,    d.concepto,    k.afecha AS feccon,    b.descripda AS bodega,    z.tipsaldo,    z.iddocu,    z.nomdoc,    z.iscero,    z.dtag,    z.swaneiva,    z.tipnum,    z.swkar,    z.swconta,    z.idsritabla,    z.ajucon,    d.serie,    d.numauto,    d.fcaducidad,    s.scel,    s.stelf,    s.saddr,    s.saddr2,    s.scode,    s.ssri,    s.sname,    s.sruc,    s.scedula,    s.tipocontri,    w.descripda AS sciudad,    s.spasaporte,    j.sname AS vendedor,    u.sname AS nombreu,    u.observ,    monedas.moname AS moneda,    d.pflag,    d.dflag,    d.idws,    q.reporte,    t.scode AS codter,    t.sname AS nombret,    z.isdevo,    z.istransfer,    z.documentos,    r.sname AS recaudador,    c2.descripda AS ciudad2,    s.diasplz,    a.sname AS cajero,    v.descripda AS provincia,    s.calific,    z.cretri,    x.descripda AS tipdocextraj,    s.issocio,    s.smail,    d.isxmlenv,    d.isxmlres,    d.isxmlcon,    z.tipoambi,    d.placa,    d.clave,    d.autoriza   FROM documents d     LEFT JOIN detagru b ON d.userstore = b.iddato     LEFT JOIN asientosdia k ON d.code = k.code     LEFT JOIN vsecudoc z ON d.idsecudoc = z.idsecudoc     LEFT JOIN docuse v1 ON z.iddocu = v1.iddocu     LEFT JOIN imagenrep q ON z.idimage = q.idimage     LEFT JOIN sujetos u ON d.idugra = u.idsujeto     LEFT JOIN periodos o ON d.pdocode = o.pdocode     LEFT JOIN monedas ON o.moneda = monedas.mocode     LEFT JOIN sujetos s ON d.cliente = s.idsujeto     LEFT JOIN sujetos j ON d.seller = j.idsujeto     LEFT JOIN sujetos r ON d.casher = r.idsujeto     LEFT JOIN detagru w ON s.sciudad = w.iddato     LEFT JOIN detagru c2 ON s.sciudad2 = c2.iddato     LEFT JOIN detagru v ON s.sprov = v.iddato     LEFT JOIN sujetos a ON d.usercaja = a.idsujeto     LEFT JOIN sujetos t ON d.sujter = t.idsujeto     LEFT JOIN sujetos f ON d.chofer = f.idsujeto     LEFT JOIN detagru x ON s.tipdocextraj = x.iddato  WHERE (v1.iddocu IN ( SELECT DISTINCT d1.iddocu           FROM gdoc g1,            dgdoc t1,            docuse d1          WHERE g1.tag::text = 'DOCELE'::text AND g1.idgdoc = t1.idgdoc AND t1.dtag::text = d1.dtag::text)) AND NOT d.isanulado;ALTER TABLE vdocxml1  OWNER TO postgres;


CREATE OR REPLACE VIEW vdocxml2 AS  
SELECT d.code,    d.fecharet AS fecha,    d.numsecue1 AS numero,    d.montoret,    d.numserie1 AS seriedoc, 
   d.numsecue1 AS numsecdoc,    d.numsecue2,    d.serieret,    d.secueret,    d.autoret,    d.fcaducret,    
   d.numserie2,    d.numauto2 AS numautadq,    d.fecha2 AS fecdocadq,    d.fcaducidad2 AS feccadadq,    
   s.descrip AS tipdoc,    u.fecha AS feccom,    u.sname,    u.sruc,    u.scedula,    u.spasaporte,    
   u.saddr,    u.saddr2,    u.sciudad,    x.smail,    u.stelf,    u.scel,    r.rubname,    r.rubtab,    
   r.valcal,    c.valor,    c.basecal AS monto,    v.nomdoc,    i.codigo AS codigoret,    d.fimpresion,   
    z.isdocele,    z.tipoambi,    u.cliente,    to_char(s.codigo::smallint::integer, 'FM09'::text) AS tipdocsustento,
    d.clave,    d.autoriza,    d.isxmlres,    d.isxmlenv,    d.isxmlcon, r.rubtab2
    FROM aneiva d   LEFT JOIN vdocuments u ON d.code = u.code     
    				LEFT JOIN cobros c ON c.code = d.code     
    				LEFT JOIN sritabla i ON c.idsritabla = i.idsritabla     
    				LEFT JOIN rubros r ON c.rubcode = r.rubcode     
    				LEFT JOIN sritabla s ON d.idtipdoc = s.idsritabla     
   ...
   WHERE r.isdocret AND z.isdocele AND NOT u.isanulado;ALTER TABLE vdocxml2  OWNER TO postgres;
